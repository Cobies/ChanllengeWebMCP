...
@Component({
  ...
  template: `
    ...
    @if (!bridge.isOpen()) {
      <div class="fixed bottom-6 right-6 z-50 flex items-center gap-2 animate-bounce-subtle">
        <button
          ...
          <svg class="w-5 h-5 text-cyan-600 group-hover:translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @if (bridge.isOpen() && !bridge.isMinimized()) {
      <div class="fixed bottom-4 right-4 sm:right-6 w-[calc(100vw-2rem)] sm:w-[460px] md:w-[500px] h-[640px] max-h-[calc(100vh-2rem)] z-50 flex flex-col rounded-2xl bg-[#fbf9f5]/95 backdrop-blur-2xl border border-slate-200/90 shadow-2xl shadow-slate-900/10 overflow-hidden animate-slide-up">
        
        <!-- Drawer Header -->
        <div class="p-3.5 px-4 bg-white/90 border-b border-slate-200 flex items-center justify-between gap-2 select-none">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-purple-600 p-[1px] shadow-sm shadow-cyan-500/20">
        ...
        <div #scrollContainer class="flex-1 p-4 overflow-y-auto space-y-3.5 custom-scrollbar">
          ...
          @if (bridge.messages().length === 0) {
            <div class="space-y-4 py-3">
              <div class="p-4 rounded-xl bg-gradient-to-b from-cyan-50/80 to-purple-50/60 border border-cyan-200/80 text-center space-y-2">
                <div class="inline-flex p-2.5 rounded-full bg-cyan-100 border border-cyan-300/60 text-cyan-700">
              ...
              <div class="space-y-2">
                <div class="text-[11px] font-bold uppercase tracking-wider text-slate-500 font-mono px-1">
                  ⚡ Quick Demo Actions:
                </div>
                  ...
                  @for (chip of promptChips; track chip.label) {
                    <button
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="text-xs font-semibold text-slate-800 group-hover:text-cyan-700 truncate">
                          {{ chip.label }}
                        </div>
                      ...
                      <svg class="w-3.5 h-3.5 text-slate-400 group-hover:text-cyan-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          @for (msg of bridge.messages(); track msg.id) {
            ...
            @if (msg.role === 'user') {
              <div class="flex justify-end">
                <div class="max-w-[85%] rounded-2xl rounded-tr-sm bg-cyan-600 text-white border border-cyan-700 p-3 text-xs shadow-xs">
                ...
                <div class="flex items-center justify-between gap-2">
                  <div class="flex items-center gap-2">
                    @if (msg.toolExecution?.status === 'success') {
                      ...
                      <span class="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-rose-50 text-rose-700 border border-rose-200 flex items-center gap-1">
                        <span>✗</span>
                        <span>{{ msg.name }}</span>
                      </span>
                    } @else {
                      <span class="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1 animate-pulse">
                        <span>⚡</span>
                        <span>{{ msg.name }}</span>
                      </span>
                    }
          ...
          @if (bridge.isGenerating()) {
            <div class="flex items-center gap-2.5 p-3 rounded-xl bg-white/90 border border-cyan-300 text-xs text-cyan-800 shadow-xs">
              <div class="flex gap-1">
                <span class="w-2 h-2 rounded-full bg-cyan-600 animate-bounce"></span>
                <span class="w-2 h-2 rounded-full bg-cyan-600 animate-bounce [animation-delay:0.2s]"></span>
    ...
    @if (previewImageUrl()) {
      <div
        ...
        <div class="relative max-w-4xl w-full max-h-[90vh] bg-white rounded-2xl border border-slate-200 p-2 shadow-2xl overflow-hidden" (click)="$event.stopPropagation()">
          <div class="flex items-center justify-between p-2 border-b border-slate-200">
            <span class="text-xs font-mono font-bold text-cyan-800">📸 WebGL Canvas Viewport Capture</span>
            <button
              (click)="closeImageModal()"
