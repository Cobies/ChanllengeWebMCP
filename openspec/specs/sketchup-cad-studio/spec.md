...
| **REQ-CAD-10** | Interactive drawing tools (`line`, `rectangle`, `circle`) MUST support mouse raycasting to ground plane (Y=0) and existing mesh planar faces with vertex/edge snapping, isolating camera controls and rendering live dimensional previews. | P0 | MUST |
